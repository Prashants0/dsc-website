import type { Metadata } from "next";
import View from "@/views/class-3-combo-dsc";

export const metadata: Metadata = {
  title:
    "Buy Class 3 Combo DSC (Sign + Encrypt) | Best for e-Tendering | SignSecure",
  description:
    "Buy Class 3 Combo Digital Signature Certificate with both Signing and Encryption on one USB token. Mandatory for e-Tendering on CPPP, GEM, IREPS & ICEGATE. Starting at Rs.1,599. eMudhra & Capricorn authorized. 1-day delivery via Aadhaar eKYC.",
  keywords:
    "combo digital signature certificate, dsc signing and encryption, class 3 combo dsc for e-tendering, sign and encrypt dsc, digital signature with encryption, combo dsc online, e-tendering dsc, encryption certificate for tenders, buy combo dsc",
};

export default function Page() {
  return <View />;
}
